      ...
      <div class="w-full max-w-md bg-white/10 backdrop-blur-md rounded-3xl p-8 shadow-2xl border border-white/20">
        <div class="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p class="text-sm text-indigo-300">Sign in to your AI knowledge assistant</p>
          </div>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="space-y-4 mt-4">
          ...
          <div class="flex items-center justify-between mt-2">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl shadow-lg shadow-indigo-500/50 font-semibold transition-all">Sign in</button>
          </div>

          <div v-if="error" class="text-sm text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-4 py-2">{{ error }}</div>
        </form>

        <div class="mt-6 text-sm text-slate-300">Don't have an account? <NuxtLink to="/signup" class="text-indigo-400 font-medium hover:text-indigo-300 transition-colors">Create one</NuxtLink></div>
      </div>
    </div>
  </div>
</template>
